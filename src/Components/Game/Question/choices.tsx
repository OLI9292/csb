...
export default class Choices extends React.Component<Props, State> {
  ...
  render() {
    const { data, isInterlude } = this.props

    const { throttled } = this.state

    const choice = (value: string, idx: number) => (
      <Button
        key={idx}
        disabled={throttled}
    ...
    const choices = this.props.data.map((d: string, i: number) => choice(d, i))

    return (
      <ScrollView>
        <ContainerView>{!isInterlude && choices}</ContainerView>
      </ScrollView>
    )
  }
}
